...
import { AddProductDrawerComponent } from '../../../../../shared/components/drawer/add-product-drawer.component';

@Component({
  selector: 'app-inventory-page',
  standalone: true,
  imports: [CommonModule, FormsModule, AddProductDrawerComponent],
  template: `
    <div class="space-y-6">
      <!-- Header -->
            ...
            <tbody class="divide-y divide-slate-100">
              @for (product of products(); track product.id) {
                <tr class="hover:bg-slate-50/50 transition-colors">
                  <td class="px-6 py-4 font-bold text-slate-700 underline decoration-indigo-200 underline-offset-4">{{ product.placa }}</td>
                  <td class="px-6 py-4">
                    <span [class]="typeClass(product.type)">{{ product.type }}</span>
                  </td>
                  <td class="px-6 py-4 font-medium text-slate-600">{{ product.model }}</td>
                  <td class="px-6 py-4 text-[11px] font-mono text-slate-400 tracking-tighter">{{ product.serial }}</td>
                  <td class="px-6 py-4 text-slate-500 font-medium">{{ product.location }}</td>
                  <td class="px-6 py-4">
                    <span [class]="statusClass(product.status)">{{ statusLabel(product.status) }}</span>
                  </td>
                </tr>
              } @empty {
                <tr><td colspan="6" class="px-6 py-12 text-center text-slate-400 text-sm">Sin productos registrados</td></tr>
              }
            </tbody>
          </table>
          <div class="px-6 py-4 bg-slate-50/30 border-t border-slate-100 text-xs text-slate-400 font-medium">
            Mostrando {{ products().length }} equipos registrados
          </div>
        </div>
      }
    </div>
...
export class InventoryPageComponent implements OnInit {
  ...
  constructor(private getAllProducts: GetAllProductsUseCase) {}
    ...
    this.getAllProducts.execute().subscribe({
      next: (data) => {
        if (data && data.length > 0) {
          ...
          this.products.set([
            { id: '1', placa: '1528-003001', type: 'Terminal', model: 'Cs10C', serial: 'SN62788911', location: 'Oficina Bogotá', status: 'assigned', createdAt: new Date(), updatedAt: new Date() },
            { id: '2', placa: '1528-003002', type: 'Terminal', model: 'Cs10C', serial: 'SN62788912', location: 'Terminal Salitre', status: 'available', createdAt: new Date(), updatedAt: new Date() },
            { id: '3', placa: 'FLM-001', type: 'Laptop', model: 'Dell Latitude 5540', serial: 'SN-DL5540-001', location: 'Oficina Bogotá', status: 'assigned', createdAt: new Date(), updatedAt: new Date() },
            { id: '4', placa: 'FLM-002', type: 'Móvil', model: 'Samsung Galaxy A54', serial: 'SN-SGA54-002', location: 'Oficina Medellín', status: 'assigned', createdAt: new Date(), updatedAt: new Date() },
            { id: '5', placa: 'FLM-003', type: 'Tablet', model: 'iPad Air 5th Gen', serial: 'SN-IPA5-003', location: 'Bodega Principal', status: 'available', createdAt: new Date(), updatedAt: new Date() },
            { id: '6', placa: 'FLM-004', type: 'Laptop', model: 'Lenovo ThinkPad', serial: 'SN-LTP-004', location: 'Taller Técnico', status: 'maintenance', createdAt: new Date(), updatedAt: new Date() },
          ]);
        }
  ...
  statusClass(status: string): string {
    ...
    switch (status.toLowerCase()) {
      ...
      case 'assigned':  return base + 'bg-blue-600';
      case 'maintenance': return base + 'bg-amber-500';
      case 'disposed': return base + 'bg-red-600';
  ...
  statusLabel(status: string): string {
    const map: Record<string, string> = {
      available: 'Disponible', assigned: 'Asignado', maintenance: 'Reparación', disposed: 'Baja'
    };
